...
#define WOBBLE_N        6
#define WOBBLE_K        0.06f
#define WOBBLE_DAMP     0.94f
#define WOBBLE_LAG      0.55f
...
#define view_is_visible(view, srv) \
	((view)->workspace == (srv)->current_workspace && \
	 ((view)->state != VIEW_MINIMIZED || (view)->anim_w > 1.0f))
...
struct view {
	...
	float wobble_dx, wobble_dy;
	float wobble_vx, wobble_vy;
	float wobble_gu, wobble_gv;
	bool wobbling;
...
struct server {
	...
	/* Offscreen FBO for motion-blurred window rendering */
	GLuint view_fbo;
	GLuint view_fbo_tex;
	int view_fbo_w, view_fbo_h;
...
static void toggle_state(struct server *srv, struct view *view, enum view_state target) {
	if (view->state == target) {
		...
		place_view(view, 0, 0, ow, oh - (target != VIEW_FULLSCREEN) * BAR_HEIGHT);
	}
}
...
static void begin_grab(struct view *view, uint32_t edges) {
	...
	if (edges) {
		...
		view->wobble_dx = view->wobble_dy = 0;
		view->wobble_vx = view->wobble_vy = 0;
		view->wobble_gu = view->frame_w > 0 ? (float)srv->grab_x / view->frame_w : 0.5f;
		view->wobble_gv = view->frame_h > 0 ? (float)srv->grab_y / view->frame_h : 0.5f;
	}
}
...
static bool handle_keybinding(struct server *srv, xkb_keysym_t sym, bool super_held, bool shift_held) {
	...
	if (srv->focused_view) {
		...
		if (sym == XKB_KEY_Left)  { set_view_state(view, VIEW_NORMAL); place_view(view, 0,      0,    ow / 2, uh);     return true; }
		if (sym == XKB_KEY_Right) { set_view_state(view, VIEW_NORMAL); place_view(view, ow / 2, 0,    ow / 2, uh);     return true; }
		if (sym == XKB_KEY_Up)    { set_view_state(view, VIEW_NORMAL); place_view(view, 0,      0,    ow,     uh / 2); return true; }
		if (sym == XKB_KEY_Down)  { set_view_state(view, VIEW_NORMAL); place_view(view, 0,      uh/2, ow,     uh / 2); return true; }
	}
...
static void render_surface_iterator(struct wlr_surface *surface, int sx, int sy, void *data) {
	...
	if (external && srv->ui_ext_shader_program) {
		...
		glUniform2f(srv->ui_ext_resolution_loc,
			(float)srv->current_output_width, (float)srv->current_output_height);
	}
...
static void render_window_frame(struct server *srv,
	...
	draw_ui_box(srv, 0, 0, frame_w, frame_h, STYLE_RAISED, frame_colors, ICON_NONE, 0);
	for (int i = 0; i < 3; i++) {
	...
	if (*title) {
		...
		draw_text(srv, title, max_tw, 0, 0, 0,
			BORDER_WIDTH + 4, BAR_PADDING + (title_h - text_h) / 2);
	}
}
...
/* Update animation and wobble physics for a single view. */
static void update_view_physics(struct server *srv, struct view *view) {
	struct wlr_box geo = view->xdg_toplevel->base->geometry;
	struct frame_insets fi = view_frame_insets(view);
	...
	/* Compute target rect based on current state */
	float tx, ty, tw, th;
	int ow = srv->current_output_width, oh = srv->current_output_height;
	switch (view->state) {
	case VIEW_MAXIMIZED:
		tx = 0; ty = 0; tw = (float)ow; th = (float)(oh - BAR_HEIGHT); break;
	case VIEW_FULLSCREEN:
		tx = 0; ty = 0; tw = (float)ow; th = (float)oh; break;
	case VIEW_MINIMIZED:
		tx = (float)ow * 0.5f; ty = (float)(oh - BAR_HEIGHT);
		tw = 0; th = 0; break;
	default:
		tx = (float)view->x; ty = (float)view->y;
		tw = (float)view->frame_w; th = (float)view->frame_h; break;
	}

	/* Animate toward target */
	view->anim_x += (tx - view->anim_x) * ANIM_SPEED;
	view->anim_y += (ty - view->anim_y) * ANIM_SPEED;
	view->anim_w += (tw - view->anim_w) * ANIM_SPEED;
	view->anim_h += (th - view->anim_h) * ANIM_SPEED;

	/* Snap when close enough */
	if ((view->anim_x - tx) * (view->anim_x - tx) +
	    (view->anim_y - ty) * (view->anim_y - ty) +
	    (view->anim_w - tw) * (view->anim_w - tw) +
	    (view->anim_h - th) * (view->anim_h - th) < 0.5f) {
		view->anim_x = tx; view->anim_y = ty;
		view->anim_w = tw; view->anim_h = th;
	}

	/* Wobble spring */
	bool grabbed = view == srv->grabbed_view && !srv->resize_edges;
	if (grabbed) {
		float cvx = (float)(srv->cursor->x - srv->prev_cursor_x);
		float cvy = (float)(srv->cursor->y - srv->prev_cursor_y);
		view->wobble_dx -= cvx * WOBBLE_LAG;
		view->wobble_dy -= cvy * WOBBLE_LAG;
	}
	view->wobbling = grabbed || view->wobble_dx != 0 || view->wobble_dy != 0 ||
	                 view->wobble_vx != 0 || view->wobble_vy != 0;
	if (view->wobbling) {
		view->wobble_vx += -WOBBLE_K * view->wobble_dx;
		view->wobble_vy += -WOBBLE_K * view->wobble_dy;
		view->wobble_vx *= WOBBLE_DAMP;
		view->wobble_vy *= WOBBLE_DAMP;
		view->wobble_dx += view->wobble_vx;
		view->wobble_dy += view->wobble_vy;
		if (!grabbed &&
		    view->wobble_dx * view->wobble_dx + view->wobble_dy * view->wobble_dy < 0.01f &&
		    view->wobble_vx * view->wobble_vx + view->wobble_vy * view->wobble_vy < 0.01f) {
			view->wobble_dx = view->wobble_dy = 0;
			view->wobble_vx = view->wobble_vy = 0;
			view->wobbling = false;
		}
...
/* Render a view (frame + client surface) into an offscreen FBO, then draw
   the composited texture at its animated position with optional wobble. */
static void render_view_composited(struct server *srv, struct view *view) {
	if (view->anim_w < 1.0f || view->anim_h < 1.0f) return;

	int fw = view->frame_w, fh = view->frame_h;

	/* Ensure FBO exists at correct size */
	if (fw != srv->view_fbo_w || fh != srv->view_fbo_h) {
		if (!srv->view_fbo) {
			glGenFramebuffers(1, &srv->view_fbo);
			glGenTextures(1, &srv->view_fbo_tex);
		}
		glBindTexture(GL_TEXTURE_2D, srv->view_fbo_tex);
		...
		srv->view_fbo_w = fw;
		srv->view_fbo_h = fh;
		glBindFramebuffer(GL_FRAMEBUFFER, srv->view_fbo);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
			GL_TEXTURE_2D, srv->view_fbo_tex, 0);
		if (srv->glyph_atlas)
			glBindTexture(GL_TEXTURE_2D, srv->glyph_atlas);
	}
	...
	int saved_w = srv->current_output_width;
	int saved_h = srv->current_output_height;
	int saved_x = view->x, saved_y = view->y;

	/* Render window into FBO at (0,0) */
	glBindFramebuffer(GL_FRAMEBUFFER, srv->view_fbo);
	glViewport(0, 0, fw, fh);
	glClearColor(0, 0, 0, 0);
	glClear(GL_COLOR_BUFFER_BIT);
	srv->current_output_width = fw;
	srv->current_output_height = fh;
	glUniform2f(srv->ui_resolution_loc, (float)fw, (float)fh);
	...
	srv->current_output_width = saved_w;
	srv->current_output_height = saved_h;
	glUniform2f(srv->ui_resolution_loc, (float)saved_w, (float)saved_h);

	/* Draw composited FBO texture at animated position/size */
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, srv->view_fbo_tex);

	float ax = view->anim_x, ay = view->anim_y;
	float aw = view->anim_w, ah = view->anim_h;

	if (view->wobbling) {
		float gu = view->wobble_gu;
		float gv = view->wobble_gv;

		struct ui_vertex tmpl = {
			.params = { (float)STYLE_TEXTURED, 0, 0 },
		};
		for (int r = 0; r < WOBBLE_N; r++) {
			for (int c = 0; c < WOBBLE_N; c++) {
				if (srv->ui_batch_count >= UI_BATCH_MAX)
					flush_ui_boxes(srv);

				float u0 = (float)c / WOBBLE_N, u1 = (float)(c + 1) / WOBBLE_N;
				float v0 = (float)r / WOBBLE_N, v1 = (float)(r + 1) / WOBBLE_N;
				...
				for (int k = 0; k < 4; k++) {
					...
					px[k] = ax + uvs[k][0] * aw + view->wobble_dx * dist;
					py[k] = ay + uvs[k][1] * ah + view->wobble_dy * dist;
				}
				...
				for (int i = 0; i < 6; i++) {
					int k = tri[i];
					srv->ui_batch[base + i] = tmpl;
					srv->ui_batch[base + i].pos[0] = uvs[k][0];
					srv->ui_batch[base + i].pos[1] = uvs[k][1];
					srv->ui_batch[base + i].box_xywh[0] = px[k];
					srv->ui_batch[base + i].box_xywh[1] = py[k];
					srv->ui_batch[base + i].box_xywh[2] = 0;
					srv->ui_batch[base + i].box_xywh[3] = 0;
				}
				srv->ui_batch_count++;
			}
		}
	} else {
		draw_ui_box(srv, (int)ax, (int)ay, (int)aw, (int)ah,
			STYLE_TEXTURED, &no_colors, ICON_NONE, 0);
	}
	flush_ui_boxes(srv);
...
static void output_frame(struct wl_listener *listener, void *data) {
	...
	/* Update physics for all views (including animating-out minimized ones) */
	struct view *view;
	wl_list_for_each(view, &srv->views, link)
		if (view->workspace == srv->current_workspace)
			update_view_physics(srv, view);

	/* Render views back-to-front */
	for_each_visible_view_reverse(view, srv)
		render_view_composited(srv, view);
	...
	/* Keep rendering while any view has active wobble or animation */
...
static void xdg_toplevel_map(struct wl_listener *listener, void *data) {
	...
	/* Initialize animation to current geometry */
	struct wlr_box geo = view->xdg_toplevel->base->geometry;
	struct frame_insets fi = view_frame_insets(view);
	view->anim_x = (float)view->x;
	view->anim_y = (float)view->y;
	view->anim_w = (float)(geo.width + fi.left + fi.right);
	view->anim_h = (float)(geo.height + fi.top + fi.bottom);
	if (view->anim_w < 1) view->anim_w = 200;
	if (view->anim_h < 1) view->anim_h = 150;
